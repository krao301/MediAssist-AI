...
from ..services.instructions import generate_first_aid_instructions, format_instructions_for_sms
...
def triage_incident(
    ...
    responder_phone = "+17166170427"     # For nearby people calls and SMS (MUST have +1 prefix)
        ...
        if location:
            ...
            try:
                ...
                for person in nearby_people[:5]:  # Limit to 5 closest people
                    print(f"      Alerting: {person['name']} ({person['distance_meters']}m away) at {responder_phone}")
                    ...
                    def email_responder(name=person['name'], distance=person['distance_meters']):
                        try:
                            result = send_emergency_email(
