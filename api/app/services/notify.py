        ...
        for contact, distance_m in contacts:
            ...
            try:
                message_obj = client.messages.create(
                    body=sms_body, from_=from_number, to=contact.phone
                )
                notified.append(contact.phone)
                print(
                    f"SMS sent to {contact.name} ({contact.phone}): {message_obj.sid}"
                )
...
def send_sms_alert(to_number: str, message: str) -> dict:
    ...
    try:
        ...
        return {"success": True, "message_sid": message_obj.sid, "to": to_number}
    except Exception as e:
...
def find_nearby_contacts(
    ...
    try:
        ...
        query = db.query(Contact).filter(
            Contact.lat.isnot(None), Contact.lng.isnot(None)
        )
        ...
        if len(all_contacts) == 0:
            ...
            try:
                ...
                print(
                    f"      DEBUG: Fresh session returned {len(all_contacts)} contacts"
                )
            finally:
                fresh_db.close()

        print(
            f"      DEBUG: Found {len(all_contacts)} total contacts in database (user_id filter: {'ON' if user_id else 'OFF'})"
        )
    except Exception as e:
    ...
    for contact in all_contacts:
        distance = haversine_distance(
            incident_lat, incident_lng, contact.lat, contact.lng
        )
        print(
            f"      DEBUG: {contact.name} is {distance:.1f}m away (radius={radius_meters}m)"
        )
...
def make_emergency_call(
    ...
    try:
        ...
        call = client.calls.create(twiml=str(response), to=to_number, from_=from_number)
...
def notify_hospital(
    ...
    try:
        ...
        call = client.calls.create(
            twiml=str(response), to=hospital_phone, from_=from_number
        )
        ...
        return {"success": True, "call_sid": call.sid, "hospital_phone": hospital_phone}
    except Exception as e:
